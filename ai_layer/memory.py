...
class CosmosMemoryManager:
    ...
    def get_user_conversations(self, user_id: str, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Starting get_user_conversations for user: {user_id}")

            # Simplified query to get all data we need
            query = """
            SELECT c.conversation_id, c.timestamp, c.user_query, c.agent_response
            FROM c 
            ...
            for i, item in enumerate(items[:3]):  # Only first 3 items for debugging
            ...
            # Group conversations in Python with enhanced error handling
            conversations = {}
            processed_count = 0
            error_count = 0
            ...
            for i, item in enumerate(items):
                try:
                    ...
                    if not conv_id:
                        logger.warning(f"Item {i} missing conversation_id: {item}")
                        error_count += 1
                        continue

                    if not timestamp:
                        logger.warning(f"Item {i} missing timestamp: {item}")
                        error_count += 1
                        continue
                    ...
                    if conv_id not in conversations:
                        conversations[conv_id] = {
                            ...
                            'message_count': 1,
                            'latest_user_query': item.get('user_query', '')[:100] + '...' if len(
                                item.get('user_query', '')) > 100 else item.get('user_query', ''),
                            'latest_agent_response': item.get('agent_response', '')[:100] + '...' if len(
                                item.get('agent_response', '')) > 100 else item.get('agent_response', '')
                        }
                        logger.debug(f"Created new conversation entry for {conv_id}")
                    else:
                        # Update existing conversation
                        if timestamp < conversations[conv_id]['first_message_time']:
                            conversations[conv_id]['first_message_time'] = timestamp
                        if timestamp > conversations[conv_id]['last_message_time']:
                            conversations[conv_id]['last_message_time'] = timestamp
                            # Update latest messages
                            conversations[conv_id]['latest_user_query'] = item.get('user_query', '')[
                                                                          :100] + '...' if len(
                                item.get('user_query', '')) > 100 else item.get('user_query', '')
                            conversations[conv_id]['latest_agent_response'] = item.get('agent_response', '')[
                                                                              :100] + '...' if len(
                                item.get('agent_response', '')) > 100 else item.get('agent_response', '')

                        conversations[conv_id]['message_count'] += 1
                        logger.debug(
                            f"Updated conversation {conv_id}, count: {conversations[conv_id]['message_count']}")

                    processed_count += 1

                except Exception as item_error:
                    logger.error(f"Error processing item {i}: {str(item_error)}")
            ...
            logger.info(f"Returning {len(result)} conversations for user {user_id}")
            return result
    ...
    def search_conversations(self,
        ...
        try:
            ...
            # Sort by timestamp in Python (descending - most recent first)
