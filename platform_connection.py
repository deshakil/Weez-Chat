...
class SlackIntegration(PlatformIntegration):
    def __init__(self):
        ...
        self.scope = ['files:read', 'channels:read', 'groups:read', 'im:read', 'mpim:read', 'users:read',
                      'users:read.email']
...
def auth_google_callback():
    ...
    try:
        ...
        if save_user_platform_token(user_email, 'google_drive', token):
            ...
            # Auto-trigger sync
            socketio.start_background_task(target=auto_sync_platform, user_email=user_email, platform='google_drive',
                                           token=token)
...
def auth_slack_callback():
    ...
    try:
        ...
        if save_user_platform_token(user_email, 'slack', token_data):
            ...
            # Auto-trigger sync
            socketio.start_background_task(target=auto_sync_platform, user_email=user_email, platform='slack',
                                           token=token_data)
